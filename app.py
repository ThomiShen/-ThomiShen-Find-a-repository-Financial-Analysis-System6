...
#在Flask 中，secure_filename 是一个用于确保文件名安全的函数，它属于 werkzeug.utils 模块。
...
#人脸识别1  这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线
...
#人脸识别2  这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线这是分隔线
@app.route('/face2', methods=['GET', 'POST'])
def face2():
    if request.method == 'POST':
        image_file = request.files['image']
        filename = secure_filename(image_file.filename)  # 确保文件名安全
        ...
        # 更新路径以指向 static/beautygirl 目录
        ref_images = [os.path.join(app.static_folder, 'beautygirls', f)
                      for f in os.listdir(os.path.join(app.static_folder, 'beautygirls'))]

        face_analysis = FaceAnalysis2()  # 创建FaceAnalysis2的实例
        similarity = face_analysis.calculate_face_similarity(filepath, ref_images)

        return render_template('face2_result.html', similarity=similarity,image_url=image_url)
...
############山海经
@app.route('/shanhai',methods=['GET', 'POST'])
def shanhai():
    return render_template('shanhai.html')
#函数 allowed_file(filename) 的目的是为了检查上传的文件的扩展名是否是允许的格式
...
import random
@app.route('/suiji', methods=['GET', 'POST'])
def suiji():
    if request.method == 'POST':
        max_number = request.form.get('maxNumber')
        student_names = request.form.get('StudentName')
        # 获取上传的文件
        file = request.files.get('file')  # 获取上传的文件
        if max_number:
            max_number = int(max_number)
            random_number = random.randint(1, max_number)
            return render_template('suiji.html', random_number=random_number)
        elif student_names:
            names = student_names.split(" ")
            random_name = random.choice(names)
            return render_template('suiji.html', random_name=random_name)
            ...
            try:
                ...
                names = []
                for row in first_sheet.iter_rows(min_row=1, values_only=True):
                    if row[0]:  # 假设学生名字在第一列
                        names.append(row[0])
                if not names:
                    flash('Excel文件中没有找到学生名字')
                    return redirect(url_for('suiji'))
                else:
                    random_name2 = random.choice(names[1:])
                    return render_template('suiji.html', random_name2=random_name2)
            except InvalidFileException:
                flash('无效的Excel文件')
                return redirect(url_for('suiji'))
    # 在 GET 请求或者没有有效表单数据的 POST 请求下也提供一个返回值
    return render_template('suiji.html')
#股票指数
@app.route('/stockindex', methods=['GET', 'POST'])
def stockindex():
    stock_analysis = LongTouAnalysis()
    stock_analysis.get_data()
    stock_analysis.apply_strategy()
    k_data=stock_analysis.print_results()
    return render_template('stockindex.html',k_data=k_data)
